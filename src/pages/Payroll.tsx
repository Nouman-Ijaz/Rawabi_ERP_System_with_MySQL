...
export default function Payroll() {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={() => setView('my_slips')}
            className={`px-3 py-2 text-xs font-medium rounded-lg transition-colors ${view==='my_slips' ? 'bg-emerald-600 text-white' : 'bg-[#1a1d27] text-slate-400 hover:text-white border border-white/5'}`}>
              ...
              <tbody>
                {periods.length === 0 ? (
                  ...
                  <tr key={p.id} onClick={() => openPeriod(p)} className="border-b border-white/5 hover:bg-white/5 transition-colors cursor-pointer">
                    ...
                    <td className="px-4 py-3"><StatusBadge value={p.status} map={PAYROLL_STATUS} border /></td>
              ...
              <div className="bg-[#1a1d27] rounded-xl border border-white/5 p-5">
                <div className="flex items-start justify-between flex-wrap gap-4">
                  <div>
                    ...
                    <StatusBadge value={activePeriod.status} map={PAYROLL_STATUS} border />
                  </div>
                  <div className="flex items-center gap-2 flex-wrap">
                    {activePeriod.slips?.length > 0 && (
                      <>
                        <button onClick={() => triggerPrint(activePeriod.slips, activePeriod)}
                          className="flex items-center gap-1.5 px-3 py-2 text-xs border border-white/10 rounded-lg text-slate-400 hover:text-white transition-colors">
                        ...
                        <button onClick={() => triggerDownload(activePeriod.slips, activePeriod)}
                          className="flex items-center gap-1.5 px-3 py-2 text-xs bg-emerald-600/20 border border-emerald-500/30 rounded-lg text-emerald-400 hover:bg-emerald-600/30 transition-colors">
                    ...
                    {canEdit && activePeriod.status==='draft' && (
                      <button onClick={doGenerate} className="px-3 py-2 text-xs bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition-colors font-medium">
                        {activePeriod.slips?.length > 0 ? 'Re-generate Slips' : 'Generate Slips'}
                      </button>
                    )}
                    {isSA && activePeriod.status==='draft' && activePeriod.slips?.length > 0 && (
                      <button onClick={doApprove} className="px-3 py-2 text-xs bg-amber-500 hover:bg-amber-400 text-black rounded-lg transition-colors font-medium">Approve</button>
                    )}
                    {isSA && activePeriod.status==='approved' && (
                      <button onClick={doMarkPaid} className="px-3 py-2 text-xs bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg transition-colors font-medium">Mark as Paid</button>
                    )}
                  </div>
                </div>
                        ...
                        {activePeriod.slips.map((s: any) => {
                          ...
                          return (
                            <tr key={s.id} className="border-b border-white/5 hover:bg-white/5 transition-colors group">
                              ...
                              <td className="px-3 py-3"><StatusBadge value={s.status} map={PAYROLL_STATUS} border /></td>
        ...
        <>
          ...
          {mySlipsLoading ? (
            ...
            <div className="space-y-3">
              {mySlips.map((s: any) => (
                <div key={s.id} className="bg-[#1a1d27] rounded-xl border border-white/5 p-4 flex items-center justify-between flex-wrap gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-500 mt-0.5">
                        ...
                        {' · '}<StatusBadge value={s.status} map={PAYROLL_STATUS} />
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-6">
                    ...
                    <button onClick={() => triggerPrint([s])}
                      className="flex items-center gap-1.5 px-3 py-2 text-xs bg-[#0c0e13] border border-white/10 rounded-lg text-slate-400 hover:text-white hover:border-white/20 transition-colors">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg>
                      Print
                    </button>
                    <button onClick={() => triggerDownload([s])}
                      className="flex items-center gap-1.5 px-3 py-2 text-xs bg-emerald-600/10 border border-emerald-500/25 rounded-lg text-emerald-400 hover:bg-emerald-600/20 transition-colors">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 10v6m0 0l-3-3m3 3l3-3M3 17V7a2 2 0 012-2h6l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2z"/></svg>
                      Download PDF
                    </button>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-[#0d0f14] rounded-2xl border border-white/10 shadow-2xl p-6">
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={()=>setShowNewPeriod(false)} className="flex-1 py-2 text-xs border border-white/10 rounded-lg text-slate-400 hover:text-white">Cancel</button>
              <button onClick={doCreatePeriod} disabled={creating} className="flex-1 py-2 text-xs font-semibold bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white rounded-lg">{creating?'Creating…':'Create Period'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showLoanForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-[#0d0f14] rounded-2xl border border-white/10 shadow-2xl p-6">
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={()=>setShowLoanForm(false)} className="flex-1 py-2 text-xs border border-white/10 rounded-lg text-slate-400 hover:text-white">Cancel</button>
              <button onClick={doCreateLoan} disabled={loanSaving} className="flex-1 py-2 text-xs font-semibold bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white rounded-lg">{loanSaving?'Saving…':'Record Loan'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {ytdData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={()=>setYtdData(null)}/>
          <div className="relative w-full max-w-sm bg-[#0d0f14] rounded-2xl border border-white/10 shadow-2xl p-6">
            <div className="flex items-center justify-between mb-5">
              <div><h2 className="text-sm font-semibold text-white">Year-to-Date Summary</h2><p className="text-[11px] text-slate-500">{ytdName} · {new Date().getFullYear()}</p></div>
              <button onClick={()=>setYtdData(null)} className="p-1.5 rounded hover:bg-white/5 text-slate-400"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/></svg></button>
            </div>
